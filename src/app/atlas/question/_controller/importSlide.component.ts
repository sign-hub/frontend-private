...
export class ImportSlideComponent extends BaseComponent implements OnInit {
  ...
  importSlide() {
    ...
    for (let i = 0; i < this.testlist.length; i++) {
      for (let y = 0; y < this.testlist[i].quests.length; y++) {
        if (!!this.testlist[i].quests[y].slides) {
          for (let k = 0; k < this.testlist[i].quests[y].slides.length; k++) {
            if (this.testlist[i].quests[y].slides[k].selected) {
              selectedNr++;
              this.questionService.requestImportSlide(this.testlist[i].quests[y].slides[k].slideId, this.questionId).subscribe(res => {
                if (res.status === 'OK') {
                  console.log(res);
                  this.dialogRef.close('OK');
                  this.openSnackBar('Updated test successful!');
